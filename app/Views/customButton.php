                  ...
                  <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6 text-end col-sm-6 col-6">
                            <p><a href="<?= getenv('app.baseURL') ?>home/dashboard" style="color:#ffff; text-decoration:none; font-size:18px;" ><i class="fas fa-arrow-left me-2"></i>Back to dashboard</a></p>
                        </div>
                    </div>
                  </div>
				  <?php
						$container_id = '.checkout-step--payment .checkout-view-header';
						$html_code = '<button type="submit" class="button button--action button--large button--slab optimizedCheckout-buttonPrimary" style="background-color: #424242;border-color: #424242;color: #fff;">PayU PAYMENTS</button>';
						$css_prop = '#payuPaymentForm>button{
	display:block;
...
<script>
	...
	$(document).ready(function(){
		...
		if(updated){
			$.toaster({ priority : "success", title : "Success", message : "Payu Payments Custom button updated for your Store,Please wait for some time and check the changes" });
		}
	});
</script>