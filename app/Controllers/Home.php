 ...
 * Class Home
 *
 * Represents a PayU setup related function to load BigCommerce
 * connector and launch configuration.
 */
class Home extends BaseController
{
	/**
	 * Index - default home page once app installed in PayU
	 * and valid segments.
	 *
					...
					if(!empty($values['invoice_id'])){
						...
						foreach($cols_data as $dbk=>$dbv){
							if(isset($values[$dbv['val']])){
								...
								}else if(($dbv['val'] == "total_amount") || ($dbv['val'] == "amount_paid")){
									$inner_array[] = $values['currency'].'. '.sprintf("%.2f",$values[$dbv['val']]);
	...
	public function getOrderDetails($authKey)
	{
		...
		if(!empty($authKey)){
			...
			if (isset($result_order_payment[0])) {
				...
				$orderDetails = \BigCommerceOrder::getBigCommerceOrder($order_id,$invoiceId);
