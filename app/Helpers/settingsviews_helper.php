 ...
 * Class SettingsViews
 *
 * Represents a helper class to connect PayU & BigCommerce 
...
class SettingsViews
{
	...
	{		
		...
		if(!empty($email) && !empty($accessToken) && !empty($storeHash)){
			...
			if (count($result) > 0) {
				...
				$builderinsert = $db->table('payu_token_validation'); 
				$builderinsert->insert($data);
				$insertID = $db->insertID();
				try{
					\SettingsViews::createCustomPages($accessToken,$storeHash,$email,$insertID,$sellerdb);
				}catch(\Exception $e){
	...
	{		
		...
		if(!empty($email_id) && !empty($validation_id)){
			...
			if (count($result) > 0) {
				$result = $result[0];
				//if(!empty($result['merchant_key']) && !empty($result['merchant_salt']) && !empty($result['auth_header'])){
				if(!empty($result['merchant_key']) && !empty($result['merchant_salt'])){
					$status = true;
				}
			}
		}
				...
				if (strpos($response->getHeader('content-type'), 'application/json') != false){
					...
					if(!empty($response)){
						...
						if(isset($response['data']['uuid'])){
							...
							$builderinsert = $db->table('payu_scripts'); 
							$builderinsert->insert($data);
							$rStatus++;
						}
					}
